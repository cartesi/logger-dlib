...
CONTRACTS_DEPLOYMENT_INFO_FILE = "./blockchain_files/concerned_contracts.yaml"
...
BASE_ABI_PATH = "/opt/cartesi/blockchain_files/"
...
def get_args():
    ...
    parser.add_argument('--base_config_file', '-b', dest='base_config_file', help="Specify the name of the dispatcher base config file to use (Default: cartesi_dapp_config.yaml)")
...
def replace_deployed_info_in_base_config(conf_yaml, dep_info_yaml):
    ...
    for concern in dep_info_yaml:
        ...
        if abi_name == conf_yaml['main_concern']['abi']:
...
def get_service_index(conf_yaml, service):
    #Getting specific index in the services array
    for i in range(len(conf_yaml["services"])):
        if (conf_yaml["services"][i]["name"] == service):
            return i
    raise Exception("{} service not found in base config".format(service))
...
with open(BASE_CONFIG_FILE, 'r') as base_yaml_file:
    ...
    custom_yaml = write_dynamic_confs(loaded_yaml["dispatcher_base"])
